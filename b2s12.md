...
import models.jugador //Importar el case class

given  CsvRowDecoder[jugador,String] = deriveCsvRowDecoder[jugador]

object calculosJugador:

  def sumatoria(datos: List[Int]): Int =
  ...
  def desviacionM(datos: List[Int]): Double =
    if datos.isEmpty then 0.0
    else
      val promedioGoles = promedio(datos)
      datos
        .map(goles => math.abs(goles - promedioGoles))
        .sum / datos.length


object informacionJugador extends IOApp.Simple:
  val filePath = Path("C:\\Users\\Usuario\\OneDrive - Universidad Técnica Particular de Loja - UTPL\\Escritorio\\Programar\\Programacion\\Practicum1.1\\TallerS12\\src\\main\\resources\\LigaPro2025-2026.csv")
  ...
  val run: IO[Unit] =
    ...
    lecturaCSV.flatMap{
      jugador =>
        //Columnas
        val colGoles: List[Int] = jugador.map(_.Goles)
        val colPJ: List[Int] = jugador.map(_.PJ)

        //OBTENCION DE LOS RESULTADOS
        val sumatoriaGoles = calculosJugador.sumatoria(colGoles)
        val promedioPJ = calculosJugador.promedio(colPJ)
        val valMaximo = colPJ.max
        val valMinimo = colPJ.min
        val desviacionMedia = calculosJugador.desviacionM(colGoles)
        val desviacionRelativa = desviacionMedia/calculosJugador.promedio(colGoles)


        //IMPRESION DE RESULTADOS
          IO.println(s" SUMATORIA DE GOLES: $sumatoriaGoles") >>
          IO.println(s" PROMEDIO PARTIDOS JUGADOS: $promedioPJ") >>
          IO.println(s" VALOR MAXIMO: $valMaximo \n VALOR MINIMO $valMinimo") >>
          IO.println(s" DESVIACION MEDIA $desviacionMedia")>>
            IO.println(s" DESVIACION RELATIVA $desviacionRelativa") >>
            IO.println(
              """
                | Podemos conocer gracias a un calculo extra como la desviacion relativa cuan alta es la desviacion
                | es decir cuanta disparidad es, y en este caso la desviacion relativa es de 0.41 lo cual nos demuestra que la
                | disparidad es muy alta.
                |""".stripMargin)

    }.handleErrorWith{ error =>
      IO.println(s"ERROR AL PROCESAR EL CSV: ${error.getMessage}")
